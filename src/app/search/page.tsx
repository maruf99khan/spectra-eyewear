      ...
      <div className="container-main py-8">
        ...
        <form onSubmit={handleSearch} className="relative mb-8">
          ...
          <input
            ...
            className="w-full border border-border pl-12 pr-4 py-3.5 text-sm bg-white outline-none focus:border-text-primary transition-colors"
          />
          {localQuery && (
            <button
              type="button"
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-1.5 text-xs text-text-primary hover:text-text-secondary transition-colors"
            >
              <SlidersHorizontal size={14} />
              Filters
              {hasActiveFilters && <span className="w-1.5 h-1.5 rounded-full bg-text-primary" />}
            </button>
            <select
              value={sort}
              onChange={(e) => updateParam("sort", e.target.value)}
              className="text-xs border border-border bg-white px-3 py-1.5 outline-none focus:border-text-primary transition-colors"
            >
          ...
          <div className={showFilters ? "lg:col-span-3" : "lg:col-span-4"}>
            {isLoading ? (
              ...
              <div className="text-center py-24 border border-border">
                ...
                <p className="text-sm text-text-muted mb-1">No results found</p>
                <p className="text-xs text-text-muted/60 mb-6">Try adjusting your search or filters</p>
                <button
                  onClick={clearFilters}
                  className="text-sm text-text-primary underline underline-offset-4 hover:text-text-secondary transition-colors"
                >
                  Clear all filters
                </button>
              </div>
                  ...
                  {filtered.map((product) => (
                    <div key={product.id}>
                      <Link href={`/products/${product.slug}`} className="group block">
                        <div className="aspect-[4/5] bg-surface-light border border-border mb-3 flex items-center justify-center text-text-muted/30 overflow-hidden">
                          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="group-hover:scale-105 transition-transform">
