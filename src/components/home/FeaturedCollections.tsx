    ...
    <section className="py-24 md:py-32">
      <div className="container-main">
        <div className="flex items-end justify-between mb-12">
          <div>
            <p className="text-text-muted text-xs uppercase tracking-[0.2em] font-medium mb-3">Curated</p>
            <h2 className="text-3xl md:text-4xl font-heading font-bold tracking-tight text-text-primary">Featured Collections</h2>
          </div>
          <Link
          ...
          {featured.map((collection, i) => (
            <div key={collection.id}>
              <Link href={`/collections/${collection.slug}`} className="group block">
                <div className="relative aspect-[4/5] bg-surface-light overflow-hidden border border-border/50 glass-card">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-white via-white/80 to-transparent">
                    <h3 className="text-xl font-heading font-bold text-text-primary mb-1">{collection.name}</h3>
                    <p className="text-sm text-text-secondary mb-3">{collection.productCount} styles</p>
