  ...
  return (
    <header
      className={`sticky top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled ? "glass-nav-solid" : "glass-nav"
      }`}
    >
      <nav className="container-main flex items-center justify-between h-16 md:h-18">
        <Link href="/" className="text-2xl font-heading font-bold tracking-tight text-text-primary" onClick={closeMenu}>
          SPECTRA
        </Link>
              ...
              >
                ...
                >
                  ...
                  {hasSubmenu && (
                    <ChevronDown
                      size={14}
                      className={`transition-transform duration-200 ${openMegaMenu === item.label ? "rotate-180" : ""}`}
                    />
                  )}
                </Link>

                {hasSubmenu && openMegaMenu === item.label && (
                  <div className="absolute top-full left-0 mt-1 w-56 glass-strong shadow-sm">
                    <div className="p-2">
                      {subcategories.map((sub) => (
                        <Link
                          key={sub.label}
                          href={sub.href}
                          className="block px-4 py-2.5 text-sm text-text-secondary hover:text-text-primary hover:bg-white/50 transition-colors rounded-sm"
                        >
                          {sub.label}
                        </Link>
                      ))}
        ...
        <div className="flex items-center gap-1">
          ...
          <Link
            href="/wishlist"
            aria-label="Wishlist"
            className="hidden sm:block p-2.5 text-text-secondary hover:text-text-primary transition-colors"
          >
            <Heart size={18} />
          </Link>
          <Link
